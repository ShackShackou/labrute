...
import { Application, Container, Graphics, Text, Assets, Sprite, Rectangle } from 'pixi.js';
...
const PixiFight: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    const clearTooltipTimers = () => {
      ...
      if (tooltipFadeTimeout !== null) {
        clearTimeout(tooltipFadeTimeout);
        tooltipFadeTimeout = null;
      }
    };

    const destroyTooltipEl = () => {
      clearTooltipTimers();
      if (tooltipEl && tooltipEl.parentNode) {
        tooltipEl.parentNode.removeChild(tooltipEl);
      }
      tooltipEl = null;
      tooltipFighterKey = null;
    };
    ...
    const ensureTooltipEl = () => {
      ...
      el.className = 'pixi-pfp-tooltip';
      el.style.position = 'fixed';
      el.style.zIndex = '10000';
      el.style.pointerEvents = 'none';
      el.style.background = '#FFF6D5';
      el.style.border = '1px solid #8B6534';
      el.style.borderRadius = '6px';
      el.style.padding = '8px';
      el.style.boxShadow = '0 2px 8px rgba(0,0,0,0.3)';
      el.style.color = '#5B4631';
    ...
    const skillTypeCache = new Map<string, string | undefined>();
    const getSkillType = (skillName: string | null | undefined) => {
      if (!skillName) {
        return undefined;
      }
      if (skillTypeCache.has(skillName)) {
        return skillTypeCache.get(skillName);
      }
      const meta = skills.find((s) => s.name === skillName);
      const type = meta?.type;
      skillTypeCache.set(skillName, type);
      return type;
    };

    const formatSkillList = (skillIds: any[] | undefined) => {
      const supers: string[] = [];
      const others: string[] = [];
      if (Array.isArray(skillIds)) {
        skillIds.forEach((raw) => {
          const skillId = typeof raw === 'number' ? raw : (raw?.id ?? raw);
          const key = SkillById[skillId as keyof typeof SkillById] ?? (typeof raw === 'string' ? raw : undefined);
          if (!key) {
            return;
          }
          const type = getSkillType(key);
          const label = t(key);
          if (type === 'super') {
            supers.push(label);
          } else {
      ...
      return { supers, others };
    };

    const setTooltipContent = (fighterData: any) => {
      ...
      const key = `fighter-${fighterData?.id ?? fighterData?.team ?? ''}`;
      if (tooltipFighterKey !== key) {
        tooltipFighterKey = key;
        const { supers, others } = formatSkillList(fighterData.skills);
        const hpCurrent = fighterData.hp ?? fighterData.currentHp ?? fighterData.maxHp ?? 0;
        const hpMax = fighterData.maxHp ?? hpCurrent ?? 0;
        const statNumber = (value: number | undefined) => Math.max(0, Math.round(value ?? 0));
        const statBarWidth = (value: number | undefined) => {
          const base = Number(value ?? 0);
          return Math.max(6, Math.min(100, Math.round(base * 2)));
        };
        const weaponNames = Array.isArray(fighterData.weapons)
          ? fighterData.weapons
            .map((weapon: any) => {
              const id = typeof weapon === 'number' ? weapon : (weapon?.id ?? weapon);
              const weaponName = WeaponById[id as keyof typeof WeaponById];
              return weaponName ? t(weaponName) : null;
            })
            .filter((skillName): skillName is string => Boolean(skillName))
          : [];
        const weaponLine = weaponNames.length ? ` � ${weaponNames.join(', ')}` : '';
        el.innerHTML = `
          <div style="font-weight:700;font-size:12px;color:#5B4631;margin-bottom:4px;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">
            ${escapeHtml(fighterData.name ?? '')}
          </div>
          <div style="font-size:10px;color:#8B6534;text-transform:uppercase;margin-bottom:4px;">
            ${escapeHtml(t('level'))} ${escapeHtml(fighterData.level ?? '')}${escapeHtml(weaponLine)}
          </div>
          <div style="font-size:10px;margin-bottom:6px;display:flex;align-items:center;gap:4px;">
            <span style="font-size:11px;color:#C0392B;">?</span>
            <span style="font-weight:600;">${statNumber(hpCurrent)}/${Math.max(1, statNumber(hpMax))}</span>
          </div>
          <div style="font-size:10px;margin-bottom:6px;">
            ${['strength','agility','speed'].map((stat) => `
              <div style="display:flex;align-items:center;margin-bottom:3px;">
                <span style="min-width:58px;color:#5B4631;font-weight:600;text-transform:uppercase;">${escapeHtml(t(stat))}</span>
                <div style="flex:1;height:6px;background:#E8D3A0;border-radius:3px;overflow:hidden;margin:0 6px;">
                  <div style="height:100%;width:${statBarWidth(fighterData[stat])}%;background:${stat === 'strength' ? '#C0392B' : stat === 'agility' ? '#2980B9' : '#F1C40F'};"></div>
                </div>
                <span style="font-weight:600;color:#5B4631;">${statNumber(fighterData[stat])}</span>
              </div>
            `).join('')}
          </div>
          ${supers.length ? `<div style="font-size:10px;margin-bottom:4px;"><span style="font-weight:700;">${escapeHtml(t('supers'))}:</span> ${supers.join(', ')}</div>` : ''}
          ${others.length ? `<div style="font-size:10px;"><span style="font-weight:700;">${escapeHtml(t('skills'))}:</span> ${others.join(', ')}</div>` : ''}
        `;
      }
      el.style.opacity = '1';
    ...
    const positionTooltip = (global: { x: number, y: number }) => {
      if (!tooltipEl) {
        return;
      }
      ...
      const rendererWidth = (app.renderer as any)?.screen?.width ?? app.renderer.width ?? W;
      const rendererHeight = (app.renderer as any)?.screen?.height ?? app.renderer.height ?? H;
      const ratioX = rendererWidth ? global.x / rendererWidth : 0;
      const ratioY = rendererHeight ? global.y / rendererHeight : 0;
      const screenX = rect.left + ratioX * rect.width;
      const screenY = rect.top + ratioY * rect.height;
      const tipRect = tooltipEl.getBoundingClientRect();
      let left = screenX - tipRect.width / 2;
      let top = screenY - tipRect.height - 22;
      left = Math.max(rect.left + 6, Math.min(left, rect.right - tipRect.width - 6));
      top = Math.max(10, Math.min(top, rect.bottom - tipRect.height - 6));
      tooltipEl.style.left = `${Math.round(left)}px`;
      tooltipEl.style.top = `${Math.round(top)}px`;
    };

    const showTooltip = (fighterData: any, global: { x: number, y: number }) => {
      if (disposed) {
        return;
      }
      clearTooltipTimers();
      setTooltipContent(fighterData);
      positionTooltip(global);
    };

    const followTooltip = (global: { x: number, y: number }) => {
    ...
    const hideTooltip = (immediate = false) => {
      ...
      if (immediate) {
        ...
        clearTooltipTimers();
        tooltipHideTimeout = window.setTimeout(finish, 140);
      }
    };

    const attachTooltipHandlers = (target: Container | null | undefined, fighterData: any) => {
      if (!target || !fighterData) {
        return;
      }
      ...
      const onEnter = (evt: any) => {
        const global = resolveGlobal(evt);
        if (global) {
          showTooltip(fighterData, { x: global.x, y: global.y });
        }
      };
      const onMove = (evt: any) => {
        const global = resolveGlobal(evt);
        if (global) {
          followTooltip({ x: global.x, y: global.y });
        }
      };
      const onLeave = () => {
        hideTooltip(false);
      ...
      const onTap = (evt: any) => {
        ...
        if (global) {
          ...
          try {
            ...
            if (bounds) {
              showTooltip(fighterData, { x: bounds.x + bounds.width / 2, y: bounds.y });
            }
          } catch {}
        }
      };
      target.on?.('pointerenter', onEnter);
      target.on?.('pointermove', onMove);
      target.on?.('pointerleave', onLeave);
      target.on?.('pointertap', onTap);
      tooltipDisposers.push(() => {
        try {
          target.off?.('pointerenter', onEnter);
        } catch {}
        try {
          target.off?.('pointermove', onMove);
        } catch {}
        try {
          target.off?.('pointerleave', onLeave);
        } catch {}
        try {
    ...
    const run = async () => {
      ...
      const mkHud = (side:'L'|'R', name:string|undefined, fighter: any) => {
        ...
        attachTooltipHandlers(portraitContainer, fighter);
        ...
        return { set, follow, nameText, updateWeapon, removeWeapon, clearWeapons, showDeathX, fullBar, hitShake, portrait: portraitContainer };
      };
      ...
      const tooltipTest = params.get('pixiTooltipTest') === '1';
      if (tooltipTest) {
        try {
          const side = params.get('pixiTooltipSide') === 'R' ? 'R' : 'L';
          const targetHud = side === 'R' ? hudR : hudL;
          const targetFighter = side === 'R' ? rightMain : leftMain;
          const portraitNode = targetHud?.portrait as Container | undefined;
          if (portraitNode && targetFighter) {
            const bounds = portraitNode.getBounds?.();
            if (bounds) {
              showTooltip(targetFighter, { x: bounds.x + bounds.width / 2, y: bounds.y + 4 });
            }
          }
        } catch {}
      }
    ...
    return () => {
      ...
      try {
        tooltipDisposers.forEach((fn) => { try { fn(); } catch {} });
        tooltipDisposers.length = 0;
      } catch {}
      try { hideTooltip(true); } catch {}
      try { destroyTooltipEl(); } catch {}
  ...
  }, [fight, scale, speedBoost, /* stageOffsetX, stageOffsetY, */ clampYMinRatio, clampYMaxRatio, leftOffsetX, leftOffsetY, rightOffsetX, rightOffsetY, approachOffset, preferVideoBackground, t]);
